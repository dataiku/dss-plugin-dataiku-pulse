...
_RUNNABLE_DIR = _BASE_DIR / "python-runnables" / "load-event-server-history"
...
def main() -> None:
    ...
    for candidate_dir in default_inputs_dirs:
        if (candidate_dir / "load-event-server-history.json").exists() and (candidate_dir / "plugin_config.json").exists():
            inputs_dir = candidate_dir
            break

    if inputs_dir is None:
        raise FileNotFoundError(
            "Could not find runnable configs (load-event-server-history.json, plugin_config.json). Tried: "
            + ", ".join(str(p) for p in default_inputs_dirs)
        )

    config_path = inputs_dir / "load-event-server-history.json"
    plugin_config_path = inputs_dir / "plugin_config.json"

    config = _load_json(config_path)
    plugin_config = _load_json(plugin_config_path)

    runnable = MyRunnable(project_key="DATAIKU_PULSE_DASHBOARD", config=config, plugin_config=plugin_config)

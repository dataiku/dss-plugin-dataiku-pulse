...
- Hardened event-server history loading so alias-source merges handle DataFrame-shaped inputs safely when duplicate or overlapping history columns are present.
- Added defensive coercion to collapse duplicate-named alias columns into a single usable series, reducing failures during history normalization and downstream processing.
- Improved runtime diagnostics with targeted warnings when duplicate alias-source columns are detected in history inputs.

### 3.0.22

- Updated the packaged plugin release to `3.0.22` by aligning `plugin.json`, the top-level `README.md`, and the v3 release notes documentation.

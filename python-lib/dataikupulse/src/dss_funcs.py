...
def _process_quality_and_persist(self, df, category, module_name, project_key, mode, results):
    file_name = "data.parquet"
    if project_key:
        file_name = f"{project_key}_data.parquet"
    df_clean, dq = _normalize_and_validate(self, df, category, module_name)
    if dq is None:
...
def run_modules(self, mode="instance", project_handle=None, client_d={}, project_key=None):
    ...
    for category, module_name, module_path in _discover_modules(dss_objs):
        ...
        results = _process_quality_and_persist(self, df, category, module_name, project_key, "client", results)
    return results
